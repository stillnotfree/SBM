...
- HTTPS-подписки, отдельные ссылки VLESS/Hysteria2 и JSON-конфиги sing-box.
...
- компактная HTTPS-подписка от совместимого серверного установщика;
- отдельная ссылка `vless://`, `hysteria2://` или `hy2://`;
- локальный JSON-профиль sing-box;
- JSON-профиль sing-box по HTTPS.

К компактной подписке или отдельной ссылке можно добавить собственный JSON с
...
Версия 1.0.0 — первый публичный релиз. Это не независимо
аудированный продукт безопасности. VLESS + REALITY + Vision и Hysteria2 + TLS
покрыты тестами совместимости, однако это не означает проверку каждого
